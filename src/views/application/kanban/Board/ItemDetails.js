...
const ItemDetails = () => {
  ...
  const { columns, comments, profiles, items, selectedItem, userStory } =
    kanban;
  ...
  if (selectedItem !== false) {
    ...
    if (selectedData?.commentIds) {
      commentList = [...selectedData.commentIds]
        .reverse()
        .map((commentId, index) => {
          ...
          return (
            <ItemComment key={index} comment={commentData} profile={profile} />
          );
        });
    }
  }
