...
const StageCard = styled(Paper)(({ theme, active }) => ({
  ...
  borderRadius: '20px',
  background: active
    ? 'linear-gradient(135deg, #dc4545 0%, #b91c1c 100%)'
    : '#ffffff',
  color: active ? '#ffffff' : theme.palette.text.primary,
  boxShadow: active
    ? '0 20px 40px rgba(220, 69, 69, 0.3)'
...
const Services = () => {
  ...
  return (
    <Wrapper>
      ...
      >
        Explore the wide range of interior design and home decor services we
        offer at Residy Home Decor.
      </SubText>
          ...
          <Grid item xs={12} sm={6} md={4} lg={3} key={index}>
            <Box
              ...
              sx={{
                ...
                background: '#fff',
                transition:
                  'box-shadow 0.18s, background 0.18s, transform 0.18s',
                '&:hover': {
      ...
      <SubText variant="subtitle1" align="center">
        Look no further than Resdiy Home Decor for end-to-end interior design
        service
      </SubText>
      ...
      <StagesSection>
        <Box sx={{ position: 'relative', zIndex: 1 }}>
          <SectionHeader
            variant="h4"
            align="center"
            sx={{ color: '#2c3e50', fontSize: '2.5rem', mb: 3 }}
          >
            Our Design Process
          </SectionHeader>
          <SubText
            variant="subtitle1"
            align="center"
            sx={{ fontSize: '1.1rem', mb: 6, color: '#5a6c7d' }}
          >
          ...
          <StagesGrid>
            {stages.map((stage, index) => (
              ...
              >
                <StageNumber active={index === activeStage}>
                  {stage.number}
                </StageNumber>
                <StageIcon active={index === activeStage}>
                  {React.createElement(stage.icon)}
                </StageIcon>
                <StageTitle active={index === activeStage}>
                  {stage.title}
                </StageTitle>
                <StageDescription active={index === activeStage}>
                  {stage.description}
                </StageDescription>
              </StageCard>
            ))}
