...
function applyStack(stack, value, dsIndex, options = {}) {
    ...
    for (i = 0, ilen = keys.length; i < ilen; ++i) {
        ...
        if (
            ...
            (singleMode || value === 0 || helpers_segment.sign(value) === helpers_segment.sign(otherValue))
        ) {
            value += otherValue;
        }
    }
...
class DatasetController {
    ...
    _update(mode) {
        ...
        meta._clip = toClip(
            helpers_segment.valueOrDefault(this.options.clip, defaultClip(meta.xScale, meta.yScale, this.getMaxOverflow()))
        );
    }
    update(mode) {}
    draw() {
...
function getRatioAndOffset(rotation, circumference, cutout) {
    ...
    if (circumference < helpers_segment.TAU) {
        ...
        const calcMax = (angle, a, b) =>
            helpers_segment._angleBetween(angle, startAngle, endAngle, true) ? 1 : Math.max(a, a * cutout, b, b * cutout);
        const calcMin = (angle, a, b) =>
            helpers_segment._angleBetween(angle, startAngle, endAngle, true) ? -1 : Math.min(a, a * cutout, b, b * cutout);
...
class PolarAreaController extends DatasetController {
    ...
    _computeAngle(index, mode, defaultAngle) {
        return this.chart.getDataVisibility(index)
            ? helpers_segment.toRadians(this.resolveDataElementOptions(index, mode).angle || defaultAngle)
            : 0;
    }
}
...
var layouts = {
    ...
    update(chart, width, height, minPadding) {
        ...
        const visibleVerticalBoxCount =
            verticalBoxes.reduce((total, wrap) => (wrap.box.options && wrap.box.options.display === false ? total : total + 1), 0) || 1;
    ...
    calculateLabelRotation() {
        ...
        if (maxLabelWidth + 6 > tickWidth) {
            ...
            labelRotation = helpers_segment.toDegrees(
                Math.min(
                    Math.asin(helpers_segment._limitValue((labelSizes.highest.height + 6) / tickWidth, -1, 1)),
                    Math.asin(helpers_segment._limitValue(maxHeight / maxLabelDiagonal, -1, 1)) -
                        Math.asin(helpers_segment._limitValue(maxLabelHeight / maxLabelDiagonal, -1, 1))
                )
            );
...
class Config {
    ...
    createResolver(scopes, context, prefixes = [''], descriptorDefaults) {
        ...
        return helpers_segment.isObject(context)
            ? helpers_segment._attachContext(resolver, context, undefined, descriptorDefaults)
            : resolver;
    }
}
...
const hasFunction = (value) =>
    ...
    Object.getOwnPropertyNames(value).reduce((acc, key) => acc || helpers_segment.isFunction(value[key]), false);
...
function _getSegmentMethod(line) {
    ...
    const useFastPath =
        !line._decimated && !line._loop && !opts.tension && opts.cubicInterpolationMode !== 'monotone' && !opts.stepped && !borderDash;
    ...
    _getLegendItemAt(x, y) {
        ...
        if (helpers_segment._isBetween(x, this.left, this.right) && helpers_segment._isBetween(y, this.top, this.bottom)) {
            ...
            for (i = 0; i < lh.length; ++i) {
                hitBox = lh[i];
                if (
                    helpers_segment._isBetween(x, hitBox.left, hitBox.left + hitBox.width) &&
    ...
    parse(raw, index) {
        ...
        index =
            isFinite(index) && labels[index] === raw
                ? index
                : findOrAddLabel(labels, raw, helpers_segment.valueOrDefault(index, raw), this._addedLabels);
...
function generateTicks$1(generationOptions, dataRange) {
    ...
    if (maxDefined && includeBounds && niceMax !== max) {
        if (
            ticks.length &&
            helpers_segment.almostEquals(ticks[ticks.length - 1].value, max, relativeLabelSize(max, minSpacing, generationOptions))
        ) {
...
function createPointLabelItem(scale, index, itemOpts) {
    ...
    const angle = Math.round(
        helpers_segment.toDegrees(helpers_segment._normalizeAngle(pointLabelPosition.angle + helpers_segment.HALF_PI))
    );
...
function parse(scale, input) {
    ...
    if (round) {
        value =
            round === 'week' && (helpers_segment.isNumber(isoWeekday) || isoWeekday === true)
