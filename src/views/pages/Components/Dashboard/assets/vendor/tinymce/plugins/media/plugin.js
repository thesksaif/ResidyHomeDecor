...
(function () {
    ...
    const getIframeHtml = (data, iframeTemplateCallback) => {
        if (iframeTemplateCallback) {
            ...
            return (
                '<iframe src="' + data.source + '" width="' + data.width + '" height="' + data.height + '"' + allowFullscreen + '></iframe>'
            );
        }
    };
    ...
    const getValue = (data, metaData, sourceInput) => (prop) => {
        ...
        const getFromValueFirst = () =>
            getFromData().bind((child) =>
                isObject(child) ? getNonEmptyValue(child).orThunk(getFromMetaData) : getFromMetaData().orThunk(() => Optional.from(child))
            );
