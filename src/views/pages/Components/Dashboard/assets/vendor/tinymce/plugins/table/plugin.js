...
(function () {
    ...
    const getRowsFromSelection = (selected, selector) => {
        ...
        return lift2(cellOpt, rowsOpt, (cell, rows) =>
            filter(rows, (row) => exists(fromDom(row.dom.cells), (rowCell) => get$2(rowCell, selector) === '1' || eq(rowCell, cell)))
        ).getOr([]);
    };
    ...
    const addMenuItems = (editor, selectionTargets) => {
        ...
        if (editor.queryCommandSupported('mceTableApplyCellStyle')) {
            editor.ui.registry.addNestedMenuItem('tablecellvalign', {
                ...
                text: 'Vertical align',
                getSubmenuItems: () =>
                    buildMenuItems(editor, verticalAlignValues, 'tablecellverticalalign', applyTableCellStyle(editor, 'vertical-align')),
                onSetup: selectionTargets.onSetupCellOrRow
            });
