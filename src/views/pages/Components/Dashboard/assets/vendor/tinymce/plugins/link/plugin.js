...
(function () {
    ...
    const getHref = (elm) => {
        var _a, _b;
        return (_b = (_a = elm.getAttribute('data-mce-href')) !== null && _a !== void 0 ? _a : elm.getAttribute('href')) !== null &&
            _b !== void 0
            ? _b
            : '';
    };
    ...
    const tryProtocolTransform = (assumeExternalTargets, defaultLinkProtocol) => (data) => {
        ...
        const suggestProtocol =
            (assumeExternalTargets === 1 && !hasProtocol(url)) || (assumeExternalTargets === 0 && /^\s*www(\.|\d\.)/i.test(url));
    ...
    const preprocess = (editor, data) =>
        findMap([tryEmailTransform, tryProtocolTransform(assumeExternalTargets(editor), getDefaultLinkProtocol(editor))], (f) =>
            f(data)
        ).fold(
