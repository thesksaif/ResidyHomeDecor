...
(function () {
    ...
    const bypassSelector = (dom) =>
        (dom.nodeType !== ELEMENT && dom.nodeType !== DOCUMENT && dom.nodeType !== DOCUMENT_FRAGMENT) || dom.childElementCount === 0;
    ...
    const run = (operation, extract, adjustment, postAction, genWrappers) => (table, target, generators, behaviours) => {
        const warehouse = Warehouse.fromTable(table);
        const tableSection = Optional.from(behaviours === null || behaviours === void 0 ? void 0 : behaviours.section).getOrThunk(
            TableSection.fallback
        );
        ...
        return output.bind((out) => {
            ...
            const tableSizing = Optional.from(behaviours === null || behaviours === void 0 ? void 0 : behaviours.sizing).getOrThunk(() =>
                TableSize.getTableSize(table)
            );
            const resizing = Optional.from(behaviours === null || behaviours === void 0 ? void 0 : behaviours.resize).getOrThunk(
                preserveTable
            );
    ...
    const getLockedColumnsWithinBounds = (startAddress, rows, lockedColumns) =>
        filter$2(lockedColumns, (colNum) => colNum >= startAddress.column && colNum <= cellLength(rows[0]) + startAddress.column);
    ...
    const insertRowAt = (grid, index, example, comparator, substitution) => {
        ...
        const newRow = cloneRow(
            ...
            (ex, c) => {
                const withinSpan =
                    index > 0 && index < rows.length && comparator(getCellElement(rows[index - 1], c), getCellElement(rows[index], c));
    ...
    const deleteColumnsAt = (grid, columns) =>
        bind$2(grid, (row) => {
            ...
            const cells = foldr(
                columns,
                (acc, column) => (column >= 0 && column < acc.length ? acc.slice(0, column).concat(acc.slice(column + 1)) : acc),
                existingCells
            );
    ...
    const elementFromGrid = (grid, row, column) => {
        ...
        return Optional.from(
            (_b = (_a = rows[row]) === null || _a === void 0 ? void 0 : _a.cells[column]) === null || _b === void 0 ? void 0 : _b.element
        )
            .filter(isEditable)
        ...
        return details
            .bind((selectedCells) => {
                ...
                const selectedColumnCells = flatten(
                    map$1(house.all, (row) => filter$2(row.cells, (cell) => cell.column >= minColRange && cell.column < maxColRange))
                );
    ...
    const adt$5 = Adt.generate([{ invalid: ['raw'] }, { pixels: ['value'] }, { percent: ['value'] }]);
    ...
    const update = (rows, columns, container, selected, annotations) => {
        ...
        return shiftSelection(selected, rows, columns, annotations.firstSelectedSelector, annotations.lastSelectedSelector).map(
            updateSelection
        );
    };
    ...
    const adt$2 = Adt.generate([{ none: ['message'] }, { success: [] }, { failedUp: ['cell'] }, { failedDown: ['cell'] }]);
    ...
    const get = (editor, isResizable) => {
        return editor.inline
            ? ResizeWire.body(SugarElement.fromDom(editor.getBody()), createContainer(), isResizable)
